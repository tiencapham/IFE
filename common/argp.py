...
def read_args():
    ...
    # training settings
    parser.add_argument('--training_frames', type=int, default=50_000_000, help='train for n environment interactions ("game_frames" in the code)')
    parser.add_argument('--record_every', type=int, default=60*60*24, help='wait at least x seconds between episode recordings (default is to use environment specific presets)')
    ...
    parser.add_argument('--env_name', type=str, default='gym:Qbert',
                        help='the gym/procgen/retro environment name, should be either gym:[name], retro:[name] or procgen:[name]\n'
                             'some gym envs:   q, Phoenix, Breakout, Qbert, Amidar, SpaceInvaders, Assault\n'
    ...
    parser.add_argument('--eps_decay_frames', type=int, default=1_000_000, help='exploration epsilon decay frames, 250_000 for rainbow paper, 1M for dopamine (when not using noisy-nets)')
    ...
    parser.add_argument('--test', type=parse_bool, default=False, help='Flag to test environment (not to stack multiple environment and allow the attention mask)')
    ...
    if args.eid is not None:
        envs = ['Adventure','AirRaid','Alien', 'Amidar', 'Assault', 'Asterix', 'Asteroids', 'Atlantis', 'BankHeist', 'BattleZone', 'BeamRider', 'Berzerk', 'Bowling', 'Boxing', 'Breakout', 'Centipede', 'ChopperCommand', 'CrazyClimber', 'Defender', 'DemonAttack', 'DoubleDunk', 'Enduro', 'FishingDerby', 'Freeway', 'Frostbite', 'Gopher', 'Gravitar', 'Hero', 'IceHockey', 'Kangaroo', 'Krull', 'KungFuMaster', 'MontezumaRevenge', 'MsPacman', 'NameThisGame', 'Phoenix', 'Pitfall', 'Pong', 'PrivateEye', 'Qbert', 'RoadRunner', 'Robotank', 'Seaquest', 'Skiing', 'Solaris', 'SpaceInvaders', 'StarGunner', 'Tennis', 'TimePilot', 'Tutankham', 'Venture', 'VideoPinball', 'WizardOfWor', 'YarsRevenge', 'Zaxxon']
